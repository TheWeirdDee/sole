...
| Duplicate claim refused on-chain | Pending | proven in tests (`second_claim_on_active_right_reverts`); not yet exercised live |
| Settlement consumes the right once | Pending | proven in tests (`replayed_settlement_reverts`); not yet exercised live |
| The venue executes only when Sole authorizes | Pending | proven in tests (`test_venue_gating.cairo`); blocked live — see status below |
...
**What's still pending, and why:** `finance`/`settle`/`settleAndRepay` go through the same `privacy_invoke` route as `claim`, but every live `finance()` attempt has failed at wallet-side simulation with a generic paymaster error and no Cairo revert reason — reproduced identically on two unrelated accounts, while `claim()` (which never calls the adapter contract) has succeeded every time. That split is consistent with the wallet's paymaster declining to sponsor a call into a contract (the `ExecutionAdapter`) it has no prior history with, rather than anything wrong with the call itself. `SoleClient.financeSelfPaid()` submits the same action via `strk20PrepareInvoke` + `executeWithProof()` instead of the sponsored route, to test that directly. Duplicate-claim and cross-venue refusal are proven in tests but not yet exercised live, since they depend on `finance` first.
